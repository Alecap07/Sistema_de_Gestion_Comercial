...
import { handleApiError } from "../functions/api.ts";
...
export default function Devoluciones() {
    ...
    const fetchDevoluciones = async () => {
        ...
        try {
            ...
            addToast("⚠️ Error fetching devoluciones", "error");
        } finally {
            setLoading(false);
        }
    };
    ...
    const fetchClientes = async () => {
        try {
            ...
            addToast("⚠️ Error fetching clientes", "error");
        }
    };
    ...
    const fetchNotasPedido = async (clienteId?: number) => {
        try {
            ...
            // Filter by clienteId if provided, otherwise show all (or empty if you prefer)
            // Showing all might be too much, so maybe only show if clienteId is provided?
            // For now, let's filter if provided, else show all.
            const filtered = clienteId ? data.filter((n: any) => n.clienteId === clienteId) : data;
            setNotasPedido(filtered);
        } catch (error) {
            handleApiError(error, "Error fetching notas de pedido", addToast);
        }
    };
    ...
    const handleSave = async (formData: Record<string, any>) => {
        ...
        if (!clienteId) {
            addToast("❌ Seleccione un cliente", "error");
            return;
        }

        // Validar items
        if (items.length === 0) {
            addToast("❌ Debe agregar al menos un producto a la devolución", "error");
            return;
        }

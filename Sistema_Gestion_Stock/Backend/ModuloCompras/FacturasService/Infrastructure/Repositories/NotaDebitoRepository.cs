...
public sealed class NotaDebitoRepository : INotaDebitoRepository
{
    private readonly IDbConnectionFactory _factory;
    public NotaDebitoRepository(IDbConnectionFactory factory) => _factory = factory;

    public async Task<NotaDebito?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        using var conn = _factory.CreateConnection();
        using var cmd = new SqlCommand("dbo.sp_NotasDebito_GetById", (SqlConnection)conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task<IEnumerable<NotaDebito>> SearchAsync(
        int? proveedorId,
        int? facturaId,
        DateTime? fechaDesde,
        DateTime? fechaHasta,
    ...
    {
        using var conn = _factory.CreateConnection();
        using var cmd = new SqlCommand("dbo.sp_NotasDebito_Search", (SqlConnection)conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task<int> CreateAsync(NotaDebito entity, CancellationToken ct = default)
    {
        using var conn = _factory.CreateConnection();
        using var cmd = new SqlCommand("dbo.sp_NotasDebito_Create", (SqlConnection)conn) { CommandType = CommandType.StoredProcedure };
    ...
    public async Task UpdateAsync(int id, NotaDebito entity, CancellationToken ct = default)
    {
        using var conn = _factory.CreateConnection();
        using var cmd = new SqlCommand("dbo.sp_NotasDebito_Update", (SqlConnection)conn) { CommandType = CommandType.StoredProcedure };
        cmd.Parameters.AddWithValue("@Id", id);
        cmd.Parameters.AddWithValue("@ProveedorId", entity.ProveedorId);
        cmd.Parameters.AddWithValue("@Fecha", entity.Fecha);
        cmd.Parameters.AddWithValue("@Motivo", entity.Motivo);
    ...
    private static NotaDebito Map(IDataRecord r) => new()
    {
